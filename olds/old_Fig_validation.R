...
A = A + scale_fill_brewer(palette="Paired") + theme_classic() + theme(legend.position="top", axis.title.x=element_blank()) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  ylab("# circRNA with BSJ >= 2 & expressed in >= 30% samples)") 
