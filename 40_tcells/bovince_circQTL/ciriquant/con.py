...
def read_libsize(path):
    with open(path, 'r') as f:
        for l in f:
            if l[:14] == '##Total_Reads:':
                break
    return int(l.strip().replace('##Total_Reads: ', ""))
...
def ciri_quant_convert_bed(path, norm_libsize = True):
    data = pd.read_csv(path, comment ='#', sep = '\t', header = None)
    data[9] = [get_attr(i) for i in data[8]]
    data[10] = [get_attr(i, 'bsj') for i in data[8]]
    order = [0,3,4,9,5,10]
    data = data.iloc[:, order]
    data.columns = ['chr', 's', 'e', 'id', 'cpm', 'bsj']
    if norm_libsize:
        libsize = read_libsize(path)/2
        data['cpm'] = (data['bsj'].astype(float)/ libsize)* 1e6
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = "Convert ciriquant gtf result to bed format (1-base coordinate format)", 
